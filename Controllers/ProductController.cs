...
namespace EcommerceAPI.Controllers
{
    ...
    public class ProductController : ControllerBase
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutProduct(int id, [FromBody] ProductUpdatedDto dto)
        {
        ...
        [HttpPatch("{id}")]
        public async Task<IActionResult> PatchProduct(int id, ProductUpdatedDto dto)
        {
            try
            {
                var updated = await _productService.PatchAsync(id, dto);
                if (updated is null) return NotFound(new { message = $"product with ID {id} not found" });
